...
2. If working, redeploy to production server ([prod-host])
...
- `rsync -avz ... root@[prod-host]:[deploy-path]/AspendoraFileShare/`
- `ssh root@[prod-host] "cd [deploy-path] && docker compose up -d --build"`
...
### Next steps
~~Deploy to production ([prod-host])~~ ✅ Done
...
~~2. Deploy to production ([prod-host])~~ ✅ Done
...
1. Synced files to production via rsync:
   ```bash
   rsync -avz --exclude 'bin/' --exclude 'obj/' ... root@[prod-host]:[deploy-path]/AspendoraFileShare/
   ```
...
```bash
rsync -avz --exclude 'bin/' --exclude 'obj/' ... root@[prod-host]:[deploy-path]/AspendoraFileShare/
ssh root@[prod-host] "cd [deploy-path] && docker compose build --no-cache && docker compose up -d"
...
### Next required steps
- Awaiting approval to deploy to the production VM (`~/code/vultr-proxmox/services/file-share-blazor`).
