...
This app reads its secrets at runtime from environment variables. **No real values live in this repo.** Values come from `~/.secrets/.env` on a developer machine, or from `[deploy-path]/.env` on the production VM ([prod-host-internal]).
...
|---|---|---|---|
| `FILESHARE_DB_PASSWORD` | Postgres password for the `fileshare_user` role used by the app container and the `postgres` sidecar. | Generated locally; stored in `[deploy-path]/.env`. | Generate new value, update `.env`, `docker compose up -d`, run `ALTER ROLE` on the running DB if rotating without a fresh volume. |
