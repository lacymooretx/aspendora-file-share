...
6. **Deploy to Production**:
   ```bash
   scp -r . root@[prod-host]:[deploy-path]
...
- **What I did**:
  1. Copied project files to `[deploy-path]` on [prod-host]
  ...
  6. Removed old Next.js file-share app (`[old-deploy-path]`) and its containers
  7. Updated nginx config at `[nginx-conf]`:
...
- **Commands run**:
  ```bash
  rsync -avz ... root@[prod-host]:[deploy-path]/
  ssh root@[prod-host] "cd [deploy-path] && docker compose up -d --build"
  ssh root@[prod-host] "cd [old-deploy-path] && docker compose down -v && rm -rf [old-deploy-path]"
  ssh root@[prod-host] "docker exec nginx-proxy nginx -t && docker exec nginx-proxy nginx -s reload"
  ```
